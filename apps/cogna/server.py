...
Write a complete first-draft chapter that:
1. Opens with a vivid scene or moment drawn directly from the recordings
2. Weaves together relevant recorded stories and follow-up conversations
3. Maintains the storyteller's distinctive voice throughout — this is their memoir, not a biography
4. Closes in a way that feels complete yet connected to the whole memoir arc

CRITICAL: Use only what was actually said. Never invent quotes, events, or details not present in the recordings. Write in a warm, literary non-fiction style — like a skilled memoir editor helping someone find the best version of their own story.
...
def _collection_db_defaults(db: Dict[str, Any]):
    db.setdefault("collection_book_bibles", {})
    db.setdefault("collection_chapters", {})
...
async def portal_invitee_assemble(storyteller_id: str, authorization: Optional[str] = Header(default=None)):
    ...
    if supabase:
        supabase.table("book_bibles").insert(record).execute()
    else:
        db["book_bibles"][bible_id] = record
        _save_db(db)
    return {
...
class MemoirBibleSaveRequest(BaseModel):
    book_bible: Optional[str] = None
    chapter_outline: Optional[str] = None


@app.put("/api/portal/invitee/{storyteller_id}/memoir/bible")
async def portal_invitee_save_bible(storyteller_id: str, payload: MemoirBibleSaveRequest, authorization: Optional[str] = Header(default=None)):
    portal_user = _auth_user(authorization)
    st_user = _verify_portal_owns_storyteller(portal_user, storyteller_id)
    user_id = st_user["id"]
...
@app.post("/api/portal/invitee/{storyteller_id}/memoir/chapters/from-outline")
async def portal_invitee_chapters_from_outline(storyteller_id: str, payload: FromOutlineRequest, authorization: Optional[str] = Header(default=None)):
    portal_user = _auth_user(authorization)
    st_user = _verify_portal_owns_storyteller(portal_user, storyteller_id)
    user_id = st_user["id"]
...
@app.post("/api/portal/invitee/{storyteller_id}/memoir/chapters/{chapter_id}/first-pass")
async def portal_invitee_chapter_first_pass(storyteller_id: str, chapter_id: str, authorization: Optional[str] = Header(default=None)):
    portal_user = _auth_user(authorization)
    st_user = _verify_portal_owns_storyteller(portal_user, storyteller_id)
    user_id = st_user["id"]
    ...
    # Get all recordings + deepening sessions
    if supabase:
...
# ----------------------------------------------------
# Collection (F-tier combined book) endpoints
# ----------------------------------------------------

def _get_f_tier_storyteller_ids(portal_user: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if supabase:
        ...
        if not f_codes:
            return []
        st_r = supabase.table("storyteller_users").select("id, first_name, last_name, email").in_("signup_code", f_codes).execute()
        return st_r.data or []
    else:
        db = _load_db()
...
def portal_collection_data(authorization: Optional[str] = Header(default=None)):
    ...
    if st_ids:
        ...
        for rec in recs:
            rec["prompt_text"] = prompt_map.get(rec.get("prompt_id") or "", "Custom question")
            parts = st_name_map.get(rec.get("storyteller_user_id"), ["Unknown"])
    ...
    if supabase:
        ...
        bbs = sorted([b for b in db["collection_book_bibles"].values() if b.get("portal_user_id") == portal_id], key=lambda x: x.get("assembled_at", ""), reverse=True)
        book_bible = bbs[0] if bbs else None
        chapters = sorted([c for c in db["collection_chapters"].values() if c.get("portal_user_id") == portal_id], key=lambda x: x.get("sort_order", 0))

    return {"recordings": all_recordings, "book_bible": book_bible, "chapters": chapters, "storyteller_count": len(storytellers)}
...
async def portal_collection_create_chapter(payload: MemoirChapterRequest, authorization: Optional[str] = Header(default=None)):
    ...
    if supabase:
        supabase.table("collection_chapters").insert(chapter).execute()
    else:
        db = _load_db(); _collection_db_defaults(db)
        db["collection_chapters"][chapter_id] = chapter
        _save_db(db)
    return {"chapter": chapter}
...
async def portal_collection_save_chapter(chapter_id: str, payload: MemoirChapterRequest, authorization: Optional[str] = Header(default=None)):
    ...
    if supabase:
        ...
        db = _load_db(); _collection_db_defaults(db)
        if chapter_id in db["collection_chapters"]:
            db["collection_chapters"][chapter_id].update(updates)
        _save_db(db)
    return {"ok": True}
